...
export class EmailService {
  ...
  private readonly fromEmail = '"Mera Bestie" <pecommerce8@gmail.com>';
  private readonly supportEmail = "support@merabestie.com";
  private readonly supportPhone = "+1-800-BESTIE";
      ...
      <div style="font-family: 'Arial', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e1e1e1; border-radius: 10px; background-color: #ffffff;">
        ...
        <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin: 20px 0;">
          ...
          <p style="margin: 5px 0; font-size: 14px; color: #555;">
            📧 Email: <a href="mailto:${
              this.supportEmail
            }" style="color: #ffb6c1;">${this.supportEmail}</a><br>
            📞 Phone: <a href="tel:${
              this.supportPhone
            }" style="color: #ffb6c1;">${this.supportPhone}</a><br>
  ...
  public async sendOrderProcessingEmail(
    ...
    const content = `
      ...
      <div style="background-color: #e8f5e8; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #28a745;">
        ...
        <p style="margin: 5px 0;"><strong>Total:</strong> $${orderDetails.total.toFixed(
          2
        )}</p>
      </div>
  ...
  public async sendOrderShippedEmail(
    ...
    const content = `
      ...
      <div style="background-color: #fff3cd; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #ffc107;">
        ...
        <p style="margin: 5px 0;"><strong>Tracking Number:</strong> <span style="background: #f8f9fa; padding: 2px 6px; border-radius: 3px; font-family: monospace;">${
          orderDetails.trackingNumber || "TRK" + Date.now()
        }</span></p>
        ${
  ...
  public async sendOrderOutForDeliveryEmail(
    email: string,
    orderDetails: OrderDetails
  ): Promise<any> {
    const content = `
  ...
  public async sendWelcomeEmail(
    ...
    return this.sendEmail(
      email,
      "Welcome to Mera Bestie - Your Style Journey Begins!",
      content
    );
  }
  ...
  public async sendPasswordResetEmail(
    ...
    return this.sendEmail(
      email,
      "Password Reset Request - Mera Bestie",
      content
    );
  }
  ...
  public async sendEmailToAllUsers(
    ...
    try {
      const users: IUser[] = await UserModel.find({}, "email name");
        ...
        await Promise.allSettled(
          batch.map((user) =>
            this.sendNewsletterEmail(
              ...
              subject,
              message,
              `unsubscribe_${user._id}_${Date.now()}` // Generate unique unsubscribe token
            )
          )
        );
